...
ros::Publisher pubbb( "/diff_drive/cmd_vel", &ayhaga);

void OMOPLATE_cb( const std_msgs::Float32& cmd_msg1){
  float omoplate_L = map(cmd_msg1.data , 0, 90,60,5 );
  float omoplate_R = map(cmd_msg1.data , 0, 90,163,135);
...
void ROTATE_cb( const std_msgs::Float32& cmd_msg3){
  float rotate_L = map(cmd_msg3.data, 0, 90, 110, 130);
  float rotate_R = map(cmd_msg3.data, 0, 90, 75, 93);
...
void BICEPS_cb( const std_msgs::Float32& cmd_msg4){
  ...
  float biceps_R = map(cmd_msg4.data, 0, 90, 115, 40);
...
void twistt (const geometry_msgs::Twist& cmd_vel){
  ayhaga.linear.x = cmd_vel.linear.x ;
  ayhaga.angular.z = cmd_vel.angular.z ;
  pubbb.publish(&ayhaga);
}
...
ros::Subscriber<geometry_msgs::Twist> twistxx("twist", &twistt );
...
//   BICEPS_R.attach(14);///
  ...
  OMOPLATE_L.write(60);  // 60 - 5
  OMOPLATE_R.write(163);  // 163 - 135
  
  SHOULDER_L.write(175);  //175-90
  SHOULDER_R.write(170);   //170 - 40 
  
  ROTATE_L.write(110); //110 - 180
  ROTATE_R.write(75);  // 75- 95
